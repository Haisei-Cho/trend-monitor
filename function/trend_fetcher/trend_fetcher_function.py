...
from utils import build_search_suffix
...
def _extract_json_object(text: str) -> dict | None:
    """テキストからJSONオブジェクトを抽出する（ブラケットカウント方式）。"""
    start = text.find("{")
    if start == -1:
        return None
...
def lambda_handler(event: dict, context: Any) -> dict:
    ...
    # 3. 除外ルール取得
    exc_kw = get_exclusion_keywords()

    # 4. スクリーニング通過トレンドの詳細ツイート取得
    items = fetch_trend_details(client, screened, exc_kw if exc_kw else None)
