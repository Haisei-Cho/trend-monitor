...
# ======================================================================
# 除外ルール (rule_id, ruleName, keywords[], apiFilter)
# ======================================================================

EXCLUSION_RULES = [
    ("retweet", "リツイート除外", [], "-is:retweet"),
    ("bot", "Bot投稿除外", [], "-is:automated"),
    ("entertainment", "エンタメ文脈除外", ["ゲーム", "アニメ", "ドラマ", "映画", "小説", "マンガ"],
     "-(ゲーム OR アニメ OR ドラマ)"),
    ("promotion", "宣伝・広告除外", [], "-is:promoted"),
    ("sns_flame", "SNS炎上除外", ["炎上商法", "炎上案件", "ネット炎上"], "-(炎上商法 OR 炎上案件 OR ネット炎上)"),
]

# ======================================================================
# 拠点マスタ（港湾・空港・幹線道路）
# (site_id, siteType, siteName, location, keywords[], infraKeywords[])
...
def get_all_items() -> list[dict]:
    ...
    for rule_id, name, kws, api_filter in EXCLUSION_RULES:
        items.append({
            "PK": f"EXCLUSION#{rule_id}", "SK": "META",
            "GSI1PK": "TYPE#EXCLUSION", "GSI1SK": f"#{rule_id}",
            "ruleName": name, "keywords": kws, "apiFilter": api_filter,
        })

    for site_id, site_type, name, location, kws, infra_kws in SITES:
        items.append({
            ...
            "keywords": kws, "infraKeywords": infra_kws,
        })
